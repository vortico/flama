...
try:
    ...
    class SessionMiddleware(starlette.middleware.sessions.SessionMiddleware):
        def __init__(self, app: "types.App", *args, **kwargs):
            super().__init__(app, *args, **kwargs)  # ty: ignore[invalid-argument-type]

        async def __call__(self, scope: "types.Scope", receive: "types.Receive", send: "types.Send") -> None:  # ty: ignore[invalid-method-override]
            return await super().__call__(scope, receive, send)  # ty: ignore[invalid-argument-type]

except ModuleNotFoundError:
    SessionMiddleware = None  # ty: ignore[invalid-assignment]
