...
from starlette_api.exceptions import ValidationError
from starlette_api.schema import formats

NO_DEFAULT = object()

FORMATS = {"date": formats.DateFormat(), "time": formats.TimeFormat(), "datetime": formats.DateTimeFormat()}
...
class Validator:
    ...
    def __init__(self, title="", description="", default=NO_DEFAULT, allow_null=False, definitions=None, def_name=None):
        definitions = {} if (definitions is None) else collections.OrderedDict(definitions)
...
class Boolean(Validator):
    ...
    values = {"true": True, "false": False, "on": True, "off": False, "1": True, "0": False, "": False}
...
class Object(Validator):
    ...
    def __init__(
        ...
        **kwargs
        ...
        properties = {} if (properties is None) else collections.OrderedDict(properties)
        pattern_properties = {} if (pattern_properties is None) else collections.OrderedDict(pattern_properties)
    ...
    def validate(self, value, definitions=None, allow_coerce=False):
        ...
        for key, child_schema in self.properties.items():
            ...
            item = value[key]
            try:
                validated[key] = child_schema.validate(item, definitions=definitions, allow_coerce=allow_coerce)
        ...
        remaining = [key for key in value.keys() if key not in set(validated.keys()) | set(errors.keys())]
            ...
            try:
                if isinstance(self.items, list):
                    if pos < len(self.items):
                        item = self.items[pos].validate(item, definitions=definitions, allow_coerce=allow_coerce)
                    elif isinstance(self.additional_items, Validator):
                        item = self.additional_items.validate(item, definitions=definitions, allow_coerce=allow_coerce)
                elif self.items is not None:
                    item = self.items.validate(item, definitions=definitions, allow_coerce=allow_coerce)
...
class Union(Validator):
    errors = {"null": "Must not be null.", "union": "Must match one of the union types."}
    ...
    def validate(self, value, definitions=None, allow_coerce=False):
        ...
        for item in self.items:
            try:
                return item.validate(value, definitions=definitions, allow_coerce=allow_coerce)
            except ValidationError:
                pass
        self.error("union")
...
class Ref(Validator):
    ...
    def validate(self, value, definitions=None, allow_coerce=False):
        ...
        return child_schema.validate(value, definitions=definitions, allow_coerce=allow_coerce)
...
class Uniqueness:
    ...
    def make_hashable(self, element):
        ...
        if element is True:
            ...
            return (
                "dict",
                tuple([(self.make_hashable(key), self.make_hashable(value)) for key, value in element.items()]),
            )
