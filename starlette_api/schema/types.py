...
from starlette_api.exceptions import ConfigurationError, ValidationError
from starlette_api.schema import validators
    ...
    def __new__(cls, name, bases, attrs):
        ...
        for key, value in list(attrs.items()):
            if key in ["keys", "items", "values", "get", "validator"]:
                msg = 'Cannot use reserved name "%s" on Type "%s", as it ' "clashes with the class interface."
        ...
        for base in reversed(bases):
            if hasattr(base, "validator"):
                properties = [
                    (key, base.validator.properties[key]) for key in base.validator.properties if key not in attrs
                ] + properties
        ...
        attrs["validator"] = validators.Object(
            def_name=name, properties=properties, required=required, additional_properties=None
        )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if args:
            ...
            if args[0] is None or isinstance(args[0], (bool, int, float, list)):
                ...
                value = {key: getattr(args[0], key) for key in self.validator.properties.keys()}
        else:
