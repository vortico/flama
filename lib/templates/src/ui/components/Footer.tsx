...
export default function Footer() {
  return (
    <footer>
      <div className="border-vortico-500/50 dark:border-vortico-300/25 border-t px-4 py-2">
        <div className="mx-auto flex max-w-7xl items-center justify-between">
          <a href="https://vortico.tech">
            <Logo logo="vortico" color="vortico" size="lg" />
          </a>
          <div className="divide-vortico-500/50 dark:divide-vortico-300/20 flex h-8 items-center justify-end divide-x">
            {social.map((item, i) => (
              <a key={i} href={item.url} className="px-2 first:pl-0 last:pr-0">
                <div className="text-primary-400 hover:text-vortico-500 dark:text-primary-300 dark:hover:text-vortico-300 h-6 w-6 transition-colors duration-200">
                  {item.icon}
                </div>
              </a>
            ))}
