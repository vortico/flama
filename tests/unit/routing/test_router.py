...
class TestCaseRouter:
    ...
    def test_eq(self, app_mock):
        route = MagicMock(Route)
        assert Router(routes=[route], app=app_mock) == Router(routes=[route], app=app_mock)
    ...
    def test_mount_declarative(self, component_mock, tags):
        ...
        sub_app_components = sub_app.components
        routes = [
            Route("/", root, tags=tags),
            Mount("/foo", app=sub_app, tags=tags),
        ]
        ...
        # Check router is created and initialized, also components are passed down
        assert isinstance(mount_with_routes_route.app, Flama)
        mount_with_routes_router = mount_with_routes_route.app
        assert mount_with_routes_router.components == sub_app_components + app.components
