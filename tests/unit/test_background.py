...
class TestCaseBackgroundTask:
    ...
    async def test_background_process_task(self, app, client, task, process_event):
        @app.route("/")
        async def test():
            return APIResponse({"foo": "bar"}, background=background.BackgroundProcessTask(task, process_event))
    ...
    async def test_background_thread_task(self, app, client, task, thread_event):
        @app.route("/")
        async def test():
            return APIResponse({"foo": "bar"}, background=background.BackgroundThreadTask(task, thread_event))
