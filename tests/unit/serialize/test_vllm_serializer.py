...
class _SharedVllmSerializerTests:
    """Shared tests for dump/info (inherited by both Cuda and Metal test classes)."""
...
class TestCaseCudaModelSerializer(_SharedVllmSerializerTests):
    @pytest.fixture(scope="function")
    def serializer(self):
        return CudaModelSerializer()
...
class TestCaseMetalModelSerializer(_SharedVllmSerializerTests):
    @pytest.fixture(scope="function")
    def serializer(self):
        return MetalModelSerializer()

    def test_load_creates_runner(self, serializer):
        model_dir = pathlib.Path("/tmp/model")
        mock_runner = MagicMock()

        with patch("flama.serialize.model_serializers.vllm.MetalModelRunner", return_value=mock_runner) as mock_cls:
            result = serializer.load(b"", model_dir=model_dir)

        assert mock_cls.call_count == 1
        config = mock_cls.call_args[0][0]
        assert config.model_config.model == str(model_dir)
        mock_runner.load_model.assert_called_once()
        assert result is mock_runner

    def test_load_not_installed(self, serializer):
        model_dir = pathlib.Path("/tmp/model")

        with patch("flama.serialize.model_serializers.vllm.MetalModelRunner", None):
            with pytest.raises(Exception, match="vllm-metal"):
                serializer.load(b"", model_dir=model_dir)

    def test_version(self, serializer):
        with patch(
            "flama.serialize.model_serializers.vllm.importlib.metadata.version", return_value="0.1.0"
        ) as mock_ver:
            assert serializer.version() == "0.1.0"
            mock_ver.assert_called_with("vllm-metal")
