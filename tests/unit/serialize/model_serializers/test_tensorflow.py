...
class TestCaseTensorflowModelSerializer:
    """Cover the tensorflow slice of :class:`flama.serialize.model_serializers.tensorflow.ModelSerializer`."""
    ...
    @pytest.mark.parametrize(
        ...
        [pytest.param({"to_json": '{"name": "m"}'}, {"name": "m"}, id="default")],
    )
    def test_info(self, model_attrs: dict, expected: dict) -> None:
        spec = SPECS[_FRAMEWORK]
