...
def test_from_object():
    ...
    assert dict(product) == {"name": "abc", "rating": 20, "created": "2020-01-01T12:00:00Z"}
...
def test_extended_from_object():
    ...
    assert dict(product) == {"name": "abc", "rating": 20, "created": "2020-01-01T12:00:00Z", "reviewer": "me"}
...
def test_setattr():
    ...
    assert dict(product) == {"name": "abc", "rating": 20, "created": "2030-02-02T13:30:00Z"}

    # Set format field, using a string.
    product.created = "2040-03-03T15:45:00Z"
    assert dict(product) == {"name": "abc", "rating": 20, "created": "2040-03-03T15:45:00Z"}
...
def test_nested():
    place = Place({"name": "Brighton", "location": {"latitude": 50.8225, "longitude": -0.1372}})

    assert place.name == "Brighton"
    assert dict(place) == {"name": "Brighton", "location": {"latitude": 50.8225, "longitude": -0.1372}}
